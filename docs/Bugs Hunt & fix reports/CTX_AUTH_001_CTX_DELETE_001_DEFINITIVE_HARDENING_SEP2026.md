...
- Parser acceptance (001b): **OPEN.** Investigated thoroughly across two separate attempts, not resolved. The first attempt (order/contiguity-based) passed its target test but broke three pre-existing tests. The second attempt sought a different signal and instead produced a **formal proof that none exists**: `test_clamps_out_of_range_scores`'s legitimate `"edge_low | 0.0\nedge_high | 1.0"` and the CTX-AUTH-001b test's injected-line scenario are structurally identical — same position, same score, same delta from the prior line — and require opposite outcomes, differing only in label text. No signal derived from structure can distinguish them; the only remaining signal is content/label-based, which is exactly the keyword-blacklist approach this template's own threat model rejects. A wire-format change (structured/tokenized output) was considered and set aside as disproportionate — it would require rewriting roughly ten tests' mock shapes and likely production callers, well beyond this task's two-named-item scope. **Re-verified from source, not assumed:** no `BrowserWorker` class exists anywhere in the codebase, and `core/web_learning/pipeline.py` — the only other plausible external-content-ingestion path — is explicitly flagged `"web_learning": False` in `core/meta/self_model.py` and invoked nowhere outside its own tests. Every live memory-write path is the system writing its own internal reasoning. **No currently traced live execution path was found that permits externally supplied content to reach memory** — a statement about what this search covered (`BrowserWorker`/`ToolResult`-shaped classes and every `memory.write()` call site reachable from `core/`), not a claim that every possible ingress mechanism in the repository was enumerated. This confirms, with concrete evidence, `docs/reports/context-compiler-remediation-register.md`'s own prediction that REM-002's parser half depends on REM-004's authority taxonomy — Context Compiler-adjacent infrastructure this task correctly excludes. A proposed (not decided) disposition is drafted at `docs/architecture/decisions/ADR_KERNEL_05_CTX_AUTH_001B_DEFERRAL_PROPOSAL.md`, recommending deferral — tracked, not blocking — contingent on the same infrastructure, with `TestCtxAuth001ParserAcceptance` left red by design as a standing trigger: the day a live external-content path is added, its continued failure is the signal to revisit this before that path ships.

**Kernel-v1.0 freeze-blocker classification (mission §34):** the live code path this half of the bug lives in (`core/cognitive/intent.py`, the K4.2 Cognitive Front-End) is confirmed Kernel-required, so no exception applies without an explicit ADR — a proposal for exactly that now exists (`ADR_KERNEL_05_CTX_AUTH_001B_DEFERRAL_PROPOSAL.md`, Status: **PROPOSED**, reviewed and endorsed by Moncif Sept 12 as technically defensible with two refinements — see that ADR's Review field — but not yet formally Accepted or part of the freeze baseline; a separate explicit sign-off is required for that). **Recommendation, reviewed but not yet formally adopted:** defer, tracked-not-blocking. The higher-leverage structural half (001a) is closed; no currently traced live execution path was found that permits externally-influenced content to reach memory (the known web-learning path is disabled and has no live callers outside its tests — see the ADR for the precise, bounded wording of this finding); and a downstream governance layer (Plan Compilation) still sits between any hijacked hypothesis and real-world effect, by this architecture's own deliberate design. This is a recommendation about present-day risk, not a claim that the underlying gap is closed — `TestCtxAuth001ParserAcceptance` stays red, and its continued failure is the explicit, automatic tripwire for the four acceptance criteria the ADR now enumerates (web_learning re-enabled, browser/tool-result ingestion introduced, any other externally-sourced write path added, or any other live untrusted-content path otherwise established).
