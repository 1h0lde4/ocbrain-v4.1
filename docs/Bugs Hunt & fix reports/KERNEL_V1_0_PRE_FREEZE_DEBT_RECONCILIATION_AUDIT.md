...
**Second update, same date, post-merge confirmation:** D10 is now actually merged (`main` HEAD `e0707eb`); re-synced and re-ran `check_drift.py` natively (no path tricks needed this time) — clean **15/15 PASS**, matching the above exactly. Both study documents confirmed byte-identical post-merge (1019 and 353 lines respectively, unchanged). One correction this pass surfaces: `KNOWN_ISSUES.md` has since assigned **DEBT-013 and DEBT-014 to two different, real findings** from `K4_2_H2_FINAL_INDEPENDENT_AUDIT.md` (a document that didn't exist in the repository during this audit's original pass — `detected_language` propagation and DRIFT-10's coverage gap, neither related to this audit's content). **Section 1.2's proposed IDs are renumbered accordingly: the RecursionGovernor gap, the `sqlite3.connect` monkeypatch, the `os.execv()` gap, and the `WorkerContext` shim should be filed as DEBT-015 through DEBT-018 (next-available), not DEBT-013–016 as originally written there.** Section 1.2's text is left as-is above as the historical record of what was proposed at the time; this note is the authoritative correction for anyone filing them now.
