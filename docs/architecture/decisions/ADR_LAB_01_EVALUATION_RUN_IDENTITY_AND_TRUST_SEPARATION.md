...
## 7. Clarification (correction pass): `evaluation_definition_id`/`version` requiredness

**Not a new decision — this section resolves a question §2's text left implicit, without changing anything §2 already decided.** A contract-correction pass asked whether every `EvaluationRun` must carry `evaluation_definition_id`+`evaluation_definition_version`. §2 lists this pair only among identities that must be *kept distinct from each other* (the DEBT-015-style collapsing concern this ADR is about); it says nothing about mandatory *presence*. Cross-checked against ADR-LAB-03: that ADR governs `EvaluatorDefinition`/`JudgeIdentity`, not `EvaluationDefinition`, and doesn't address this either.

Conclusion, for the record: **the reference is optional**, and the specific condition under which it should be absent is documented on `EvaluationRun.evaluation_definition_id` itself (run.py) — in short, a run whose results are each fully specified by their own evaluator identity needs no `EvaluationDefinition`; it earns its keep specifically when two or more evaluator implementations need to be treated as alternative answers to the same measurement question. This does not amend §2's decision; it fills a gap §2 left open.
