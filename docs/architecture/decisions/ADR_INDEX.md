...
| ADR-KERNEL-05 | CTX-AUTH-001b deferral recommendation -- formal proof that no structural parser signal can close the gap without breaking pre-existing tests or resorting to a rejected blacklist approach; re-verified no live external-content-ingestion path exists today; recommends tracked-not-blocking pending Context Compiler's authority taxonomy | **PROPOSED** — awaiting Moncif's decision | `ADR_KERNEL_05_CTX_AUTH_001B_DEFERRAL_PROPOSAL.md` |
