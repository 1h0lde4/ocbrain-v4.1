...
**Migration status (ADR-KERNEL-01, Aug 29, 2026): completed.** This ADR's original "zero live consumers" claim was accurate when written, and became false through ordinary subsequent feature development with no mechanism to catch the drift: six worker classes built after this ADR (`CapabilityExecutorWorker`, `CuratorWorker`, `EvaluatorWorker`, `PlannerWorker`, `ReflectionWorker`, `SupervisorWorker`) all inherited `AbstractCognitiveWorker`'s abstract `_run(self, context: WorkerContext)` signature, and `ExecutionRuntime.invoke()` was explicitly converting every `ExecutionContext` to a `WorkerContext` via `to_worker_context()` "for backward compatibility" before calling any worker — a compatibility bridge whose own code comment marked it "will be removed after K2.4," which completed in July without the bridge being removed. Kernel Blocker B (identified in `docs/studies/OCBRAIN_KERNEL_COMPLETION_CANONICAL_PLAN.md`, confirmed still open by direct code inspection on Aug 29) was exactly this gap.

Resolved by completing the migration this ADR always intended: `ExecutionContext` already carried `task_id`/`query`/`recursion_depth` bridge properties (built for exactly this purpose, never used); a `parameters` bridge property was added to complete the set. All seven worker classes' `execute()`/`_run()`/`emit_progress()` signatures now take `ExecutionContext` directly. `ExecutionRuntime.invoke()` no longer calls `to_worker_context()` — verified by direct code read, not assumed. Zero regressions: full suite unchanged at 34 pre-existing environment-only failures (1,343 passed, including 12 new tests proving the actual runtime path via `isinstance` checks, not merely that the type annotations changed). `WorkerContext` itself is retained, not deleted, pending its own test-fixture references being updated or the class being judged low-risk enough to delete outright — see ADR-KERNEL-01 for the exact remaining reference count.
