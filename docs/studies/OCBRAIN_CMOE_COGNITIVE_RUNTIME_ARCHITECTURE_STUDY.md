...
>
> **Reconciliation pass, Sept 1, 2026:** a companion document, `docs/studies/OCBRAIN_CMOE_VERIFICATION_ARCHITECTURE_STUDY.md`, landed independently and was checked against this one — its central and most load-bearing claims (a live `core/workflow/runtime.py` bug reducing goal-completion to a single node's success flag; `ExecutionOutcome.is_success` conflating partial output with success) were independently re-verified by direct code read, not merely trusted from its own tags. Its proposed placement — a thin Evidence/Verification primitive at the Kernel/execution layer, consumed by C-MoE rather than owned by it — matches this document's own §69 stub exactly. §69 and the misattributed quote in §32 were updated with pointer-style corrections; no architectural boundary in this document changed as a result.
...
**[STUDY, reconciled]** A companion document, `docs/studies/OCBRAIN_CMOE_VERIFICATION_ARCHITECTURE_STUDY.md`, independently investigated the boundary this section stubs and reached the same placement: a thin Evidence/Verification primitive belongs at the Kernel/execution layer, not inside C-MoE, with C-MoE consuming its output through slots this study already reserved (`verification_status`, the verification-confidence dimension in §22, the `VERIFYING` state in §31) rather than owning verification logic itself. It also independently confirmed a live, C-MoE-independent Kernel bug worth naming here because it directly motivates why this boundary matters in practice, not merely in principle: `core/workflow/runtime.py`'s workflow-completion check currently reduces to the last node's own success flag (`success = last_result.success if last_result is not None else True`, confirmed by direct read), and `ExecutionOutcome.is_success` treats `COMPLETED_WITH_PARTIAL_OUTPUT` as equal to `SUCCESS` (also confirmed) — neither checks the result against the goal's actual constraints. This is exactly the gap §32's `COMPLETED_WITH_WARNINGS` cap already exists to contain at the C-MoE layer; it is a Kernel-layer correctness issue below this study's own scope, not a finding this study needs to design a fix for. No boundary change to this section: the companion study confirms the stub, it does not replace it.
