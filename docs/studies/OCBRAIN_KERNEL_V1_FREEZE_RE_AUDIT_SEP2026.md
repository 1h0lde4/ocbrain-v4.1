...
1. **CTX-AUTH-001b (Parser / Authority Acceptance):** `_parse_hypotheses()` in `core/cognitive/intent.py` accepts context-injected hypothesis lines (e.g. `novel:CONTEXT_SENTINEL_INJECTED | 0.55`) with no machine-verifiable authority check or provenance tracing. While `ADR-KERNEL-06` (Verifiable Hypothesis Provenance) has been accepted in principle, its enforcement mechanism is not yet implemented in code.
...
Until explicit governance decisions or technical remediations close these three gaps, Kernel v1.0 cannot be certified as frozen.
...
* **Current State:** **OPEN (Intentionally Red Test)**.
* **Empirical Evidence:** Running `pytest tests/core/cognitive/test_intent_security.py` fails on `test_injection_shaped_completion_line_is_not_accepted_as_a_hypothesis`. An injection payload inside completion text generates a valid `IntentHypothesis(label='novel:CONTEXT_SENTINEL_INJECTED', score=0.55)`.
* **Root Cause:** `IntentHypothesis` contains only `label`, `score`, and `embedding_ref`. It lacks an authority level or provenance tracking field. `_parse_hypotheses()` parses completion lines purely via regex without verifying whether the candidate was generated by system logic or injected via prompt context.
* **Required Decision / Resolution:**
  - `ADR-KERNEL-06` establishes the invariant that **authority is inherited, never inferred**.
  - To close CTX-AUTH-001b, `_parse_hypotheses()` must verify citations against assembled prompt context blocks (`Context.blocks`), rejecting untrusted/unattributed hypothesis candidates.
...
## 2. Status of Previously Resolved Blockers
...
## 3. Comprehensive Active Technical Debt Triage (DEBT-002 to DEBT-028)
...
| **DEBT-019** | Security | Medium | Context Engineering audit remediation register tracking (REM-001 to REM-015). | Partially Blocking (CTX-AUTH-001b) |
...
## 4. Test Suite Execution & Environment Health
...
* **Execution Results:**
  ...
  - **Skipped:** 14
  - **Failures:** **4**
    1. `tests/core/cognitive/test_intent_security.py::TestCtxAuth001ParserAcceptance` (Expected security invariant failure for CTX-AUTH-001b).
...
## 5. Required Action Items for Kernel v1.0 Freeze Sign-Off

To achieve official Kernel v1.0 Freeze (`FREEZE_READY`), the following explicit actions/decisions are required:

1. **Decision on CTX-AUTH-001b:** Implement `ADR-KERNEL-06` authority verification in `_parse_hypotheses()` to turn `TestCtxAuth001ParserAcceptance` green, OR formally sign off on deferring CTX-AUTH-001b to post-freeze Context Compiler work.
...
4. **Tagging:** Retag `v1.0.0` on `main` once decisions 1–3 are finalized.
