...
`docs/Bugs Hunt & fix reports/FALSE_COMPLETION_KERNEL_AUDIT_PRE_IMPLEMENTATION_REPORT.md` traces a live success-boolean chain (`core/workflow/runtime.py:530`→`core/workers/evaluator.py:172` -- corrected Sept 9, 2026; the file name previously cited here was a citation error, not a second real bug site) that let partial output reach `SUCCESS`. A companion study, `docs/studies/OCBRAIN_CMOE_VERIFICATION_ARCHITECTURE_STUDY.md`, reconciles a proposed Evidence/Verification layer against the C-MoE study's existing stub fields (`verification_status`, `REUSABLE`/`STALE`, the capped task-status enum) rather than replacing them. **Updated Sept 9, 2026:** implemented, not merely investigated or proposed -- a `CompletionStatus`/`CompletionReason` gate now sits at the site above; see `KNOWN_ISSUES.md` DEBT-020 for full technical detail and what remains (streaming parity, commit/rollback write-up, the full adversarial test matrix; not yet merged to `main`).
