...
**Last synchronized:** Sept 12, 2026, branch reconciliation (per Moncif's explicit instruction, before the fresh freeze audit). Merged 5 branches into canonical main: two research-only docs (external-research-brainstorm, sandbox-execution-fabric), one architecture study (Browser/Web Agent, FINAL version only), the Kernel v1.0 Definitive Closure Audit (historical record -- predates this session's DEBT-016/003/020), and the context-security-findings fix branch (real code: CTX-DELETE-001/CTX-CACHE-001/CTX-AUTH-001a fixed, CTX-SCOPE-001 substantially fixed but kept open pending 3 newly-found live callers). Added DEBT-024 (SupervisorWorker retry unreachable) and DEBT-025 (interface/api.py has no authentication) -- renumbered from this pass's original local DEBT-021/022 after a same-day collision with origin/main's already-public DEBT-021/022/023 (Sandbox/Execution Fabric findings, see immediately below), both independently re-confirmed against current main, not just cited from the closure audit. Kept separate (not merged): feature/verification-critic-evidence-phase-c (explicitly excluded from Kernel v1.0 scope) and eval-lab/research-and-architecture (large self-contained subsystem, proposed-only ADRs, not reviewed). Revisited and corrected this session's own Sept 6 freeze-audit reasoning about CTX-* severity -- see CURRENT_STATE.md's reconciliation entry for the full account. Full suite: 1,411 passed / 35 failed (34 environmental + CTX-AUTH-001b only), down from 39. Fresh Kernel v1.0 freeze audit is next. **Last synchronized:** Sept 12, 2026, Sandbox/Execution Fabric Phase 1 started (`sandbox-fabric` branch, commit `1ec6b7c` — see the new section below, and `CURRENT_STATE.md`/`KNOWN_ISSUES.md` for full detail). Isolated from every phase below; no Verification/C-MoE/Kernel roadmap item touched or re-sequenced. Prior sync Sept 6, 2026, DEBT-020 fix (Kernel v1.0 freeze blocker, per Moncif's explicit decision) -- `docs/architecture/decisions/ADR_KERNEL_04_FALSE_COMPLETION_FIX.md`. Remediated to the three approved points only; resumability, generalized coverage infrastructure, continuous verification, and the Verification/Critic/Evidence branch explicitly excluded. Next per Moncif's stated sequence: fresh freeze audit, then debt/register reconciliation, then freeze decision, then the Hail-Mary reliability campaign. 1,406 passed / 39 failed. Prior sync Sept 6, 2026, Kernel v1.0 freeze audit -- `docs/studies/OCBRAIN_KERNEL_V1_FREEZE_AUDIT_SEPT_2026.md`. Verdict: NOT_FREEZE_READY, pending Moncif's classification of DEBT-020 (false completion) as a blocker or explicitly-deferred fast-follow -- the only open question; everything else this audit checked is resolved or confirmed non-blocking. Watchdog (DEBT-016) and Checkpoint/resume (DEBT-003), this session's approved batch, are both done. 1,367 passed / 39 failed. Sept 6, 2026, tracking-doc sync (this session — the furthest-behind of the three tracking docs; a fresh clone found ADR-KERNEL-01's own follow-up correction, the Context Engineering Security Audit, and the false-completion Kernel audit all missing here, none reflected below. Rebased twice, mid-session: concurrent sessions landed the DEBT-016 and then DEBT-003 resolutions, both immediately below, while this sync was in progress). Adds three new sections below — ADR-KERNEL-01 Follow-Up, Context Engineering Security Audit, and False-Completion Kernel Audit — the middle one reproducing its remediation register's own Tier 1–4 prioritization as this document's actual next-step content, not merely summarizing it elsewhere. No prior claim in this document re-evaluated or found wrong; this sync only adds what was missing. Prior sync Sept 5, 2026 (DEBT-003 resolved -- Checkpoint/resume, `docs/architecture/decisions/ADR_KERNEL_03_CHECKPOINT_RESUME.md`. Both items in this session's approved batch, Watchdog (DEBT-016) and Checkpoint/resume (DEBT-003), are now done; the Kernel freeze audit is the next planned step. 1,365 passed / 39 failed, zero regressions.) Prior sync Sept 5, 2026 (DEBT-016 resolved — Watchdog reconciliation, `docs/architecture/decisions/ADR_KERNEL_02_WATCHDOG_UNIFICATION.md`; per Moncif, this session follows this roadmap's existing sequencing — Watchdog/DEBT-003/DEBT-016 before the freeze audit, Verification/Critic/Evidence integration stays post-freeze/post-C-MoE — rather than an alternative ordering proposed in this session's opening prompt. DEBT-003 is this session's next step. 1,356 passed / 39 failed, zero regressions.) Prior sync Aug 29, 2026 (ADR-KERNEL-01 — both Kernel v1.0 freeze blockers from the Canonical Kernel Completion Plan resolved same day: `root_operation_id`/`attempt_id` identity threading, and completion of the `WorkerContext`→`ExecutionContext` migration ADR-001 had declared but never finished. See `CURRENT_STATE.md` and `docs/architecture/decisions/ADR_KERNEL_01_ROOT_OPERATION_IDENTITY_AND_EXECUTION_CONTEXT_MIGRATION.md` for full detail. 1,343 passed / 34 failed, zero regressions; prior sync Aug 27, 2026 (added the Execution Reliability Track section — watchdog/execution-budget baseline merged from `feature/execution-progress-inspection`, orthogonal to and not gating/gated-by the Cognitive Front-End Phase below; see `KNOWN_ISSUES.md` DEBT-015/DEBT-016; prior sync Aug 22, 2026 K4.2-H2-D10 integrated, H2 formally closed after the independent audit's single blocker was resolved; prior sync Aug 22, 2026 K4.2-H2 complete — D3/D7/D11/D12 all merged, plus the out-of-band ADR-K4.2-H-13 fix; prior sync Aug 18, 2026 K4.2-H2 D10 pre-H2 drift baseline capability built and captured; D3/D7/D11/D12 set up for four-way parallel zero-contact Claude implementation — see `docs/architecture/h2_packets/README.md`; prior sync Aug 17, 2026 K4.2-H1 FROZEN — independent freeze review passed all 16 gates, Moncif signed off; H2 entry gate satisfied, H2 not yet started — see `docs/Bugs Hunt & fix reports/K4_2_H1_FINAL_FREEZE_REVIEW.md`; prior sync Aug 16, 2026 added K4.2-H1 — Contract Evolution Foundation; prior sync July 30, 2026 added Packet 09 — all 9 packets now complete, see note below; prior sync same day added Packet 08; prior sync same day added missing K4.2.6/K4.2.7 rows and Packet 07; prior sync July 29, 2026 corrected K4.2.4/K4.2.5 and added Plan Compilation; prior sync July 24, 2026 added the Cognitive Front-End Phase for K4.2.1–K4.2.3; prior sync July 22, 2026 K3 status correction; prior full sync July 18, 2026)
...
**Merged and frozen:** configurable execution budget replacing the old flat timeout; watchdog/progress monitoring (two implementations sharing one decision core as of Sept 5, 2026 — see `KNOWN_ISSUES.md` DEBT-016 resolution and `docs/architecture/decisions/ADR_KERNEL_02_WATCHDOG_UNIFICATION.md`); SSE for long-running responses; the `use_k42_frontend` routing flag.

**Researched and proposed, not implemented:** stable operation identity across retries, `ExecutionAttempt`, `ExecutionSnapshot`, extended failure classification, a `RecoveryDecision` policy layer, idempotency semantics. (Node-boundary checkpoint persistence specifically -- this list's original sub-item -- was resolved narrowly on Sept 5, 2026 as DEBT-003, using the existing `EventStream` WAL primitive directly rather than this broader schema; see `docs/architecture/decisions/ADR_KERNEL_03_CHECKPOINT_RESUME.md`. The rest of this architecture remains proposed only.) Full research (Temporal, LangGraph, Restate, and recent academic work, honestly sourced) and the proposed architecture are in `docs/reports/WATCHDOG_EVOLUTION_RESEARCH_AND_ARCHITECTURE_REPORT.md`. Per this project's Architecture Freeze Principle, this should become its own ADR before implementation begins — tracked as `KNOWN_ISSUES.md` DEBT-015, explicitly deferred pending that review, not scheduled against any other phase's completion.
...
## ADR-KERNEL-01 Follow-Up — Complete (Sep 2, 2026)

Re-verified ADR-KERNEL-01's own claimed residual gap (the Orchestrator's re-plan loop allegedly creating a fresh `Goal()` on recovery, which would have lost `root_operation_id`) against current code and found it doesn't exist as described: the only `Goal()` construction site is called once per `handle()` invocation, and the impasse re-plan loop reuses that same object on every retry, so `root_operation_id` was already, trivially preserved. No code change needed for this specific item — ADR-KERNEL-01's own text was corrected in place (struck through, not deleted), proven with 2 new tests against the real `Orchestrator.handle()` path. `WorkflowNode` scope decision and the `WorkerContext`→`ExecutionContext` migration were both re-confirmed unchanged and still holding. Full suite: 1,347 passed / 36 failed (34 pre-existing `huggingface.co` failures plus 2 pre-existing, intentionally-red `CTX-AUTH-001` tests, unrelated), zero regressions from this work. See `CURRENT_STATE.md` and `docs/architecture/decisions/ADR_KERNEL_01_ROOT_OPERATION_IDENTITY_AND_EXECUTION_CONTEXT_MIGRATION.md` for full detail.

---

## Context Engineering Security Audit — Six Phases Complete (Aug 31 – Sep 4, 2026)

A dedicated audit sequence — independent of and not gating the Cognitive Front-End/Kernel phases above — traced context/memory data across every reachable path and closed with a formal architecture decision plus a trackable remediation register. See `CURRENT_STATE.md`'s new Context Engineering Security Audit section for the full findings table (CTX-AUTH-001/SCOPE-001/CACHE-001/DELETE-001/EXPORT-001, plus two confirmed non-issues: artifact isolation and tool-result isolation) and `KNOWN_ISSUES.md` DEBT-019 for the debt-register entry. Key decision: shared cognitive memory remains the correct model; current evidence does not support new task/execution-level memory isolation as part of Context Compiler adoption.

**What comes next, per `docs/reports/context-compiler-remediation-register.md`'s own tier structure — reproduced here as this document's actual next-step content, not a summary of it:**

- **Tier 1 — must fix before Context Compiler adoption:** hash the full prompt (or hash-then-compress instead of compress-then-hash) in `cached_generate()` (CTX-CACHE-001); structural delimiting + explicit untrusted-data framing at Intent Hypothesis prompt construction, as part of the `LLMContext` materialization design (CTX-AUTH-001's root cause); design work on two prerequisite architecture gaps first — structured `Context` discarded before any consumer, and no authority taxonomy yet.
- **Tier 2 — must fix before Kernel/C-MoE integration:** re-evaluate global-pool memory scoping once genuine multi-worker parallelism exists — explicitly not before; do not pre-emptively redesign now.
- **Tier 3 — hardening, real and tracked, not blocking:** thread a scope parameter through `ContextMemory`'s `save()`/`last_n()`/the `turns` schema (CTX-SCOPE-001); fix `UnifiedMemory.delete()`'s silent-failure return (CTX-DELETE-001); checksum/signature validation + path restriction for `brain_export.py`'s `import_module()` (CTX-EXPORT-001); remove or wire in dead code (`core/runtime/efficiency.py`'s `PromptCache`/`cost_aware_call`, unreconciled with `core/prompt/cache.py` — the same duplication pattern DEBT-016, above, just resolved for the watchdog); a stale comment fix in `core/orchestrator.py`; the `KNOWN_ISSUES.md` DEBT-013/DEBT-017 doc-sync fix (**applied this session — see `KNOWN_ISSUES.md`'s Sept 6 sync entry**).
- **Tier 4 — deferred research:** a dedicated audit pass for five confirmed-live subsystems never in scope for any of the six phases (`core/memory/mem_vault.py`, `cognitive_vault.py`, `core/web_learning/pipeline.py`, `core/memory/consolidation/consolidator.py`, `core/shadow/collector.py`); whether `ContextMemory` should be absorbed into Context Compiler's scope or hardened in place as a separate subsystem; compression (LLMLingua-style) cost/benefit for this pipeline specifically; whether the apparent second export/import router (`interface/api.py` vs `core/brain_api.py`) is intentional or accidental duplication.

None of Tier 1–4 is scheduled against any other phase above. `docs/reports/context-compiler-remediation-register.md` is the one to update as items move — this section should not need editing as work progresses, per the register's own stated purpose.

---

## False-Completion Kernel Audit — Research Complete, Classification Pending (Sep 2, 2026)

`docs/Bugs Hunt & fix reports/FALSE_COMPLETION_KERNEL_AUDIT_PRE_IMPLEMENTATION_REPORT.md` traces a live success-boolean chain (`core/runtime/execution_runtime.py:280`→`:320`→`core/workers/evaluator.py:172`) that lets partial output reach `SUCCESS`. A companion study, `docs/studies/OCBRAIN_CMOE_VERIFICATION_ARCHITECTURE_STUDY.md`, reconciles a proposed Evidence/Verification layer against the C-MoE study's existing stub fields (`verification_status`, `REUSABLE`/`STALE`, the capped task-status enum) rather than replacing them. Investigation and proposal only — no code changed, no K-number assigned, no freeze blocker declared. **Both reports route classification back to the project owner rather than deciding it here**; this section exists so the finding has a place in the roadmap once that decision is made, not to schedule it preemptively. See `KNOWN_ISSUES.md` DEBT-020.

---

## Sandbox / Execution Fabric — Phase 1 Started (Sept 12, 2026)

Isolated work on `sandbox-fabric`, deliberately separate from every phase above per its own reconciliation's explicit scoping (no Verification/C-MoE/Kernel code touched). This is the Task Runner from `PROJECT_INSTRUCTIONS.md` §3, previously 100% aspirational — confirmed by repo-wide search: no `Runner`/`TaskRunner` class existed anywhere in the codebase before this work. An external deep-research report on sandbox/execution-fabric design (`docs/research/sandbox-execution-fabric/deep-research-report.md`) was fact-checked (5 corrections: nsjail license and staleness claim, Firecracker device count, an OpenHands/Sysbox license conflation, a Daytona mislabel) and reconciled against the existing codebase (`docs/architecture/sandbox-execution-fabric-existing-code-reconciliation.md`) before any implementation began, per this project's read/reconcile/design discipline — four naming-collision decisions recorded there (`Sandbox*` under `core/sandbox/`, not `Execution*`; `AdmissionGate`, not `ValidationGate`; "extension," not "capability," for GitHub-sourced code; `SandboxBackend`, not bare `Runtime`) and two reuse targets identified (`core/skills/skill_interface.py`'s dormant `SkillExecutionConfig`; the existing `EventStream`).

Phase 1 (commit `1ec6b7c`) built and empirically verified: contracts, the `SandboxBackend` interface, `AdmissionGate`, an `EventStream` bridge, and a real namespace/cgroup-v1 backend — the "runc rootless" option from the source report's own §13 point 4, chosen because `docker`/`runc`/`podman` were all verified absent in the build/test environment rather than assumed available. 36/36 tests passing (adversarial: path/symlink escape, network-deny-by-default, memory/pids limits, timeout, cancellation, artifact hashing), including two real bugs found and fixed by those tests, not hidden by them. A `DockerBackend` on the same interface, seccomp-bpf filtering, and `allowed_hosts` network-allowlist enforcement are explicitly deferred, not silently skipped — see `KNOWN_ISSUES.md` DEBT-021/DEBT-022/DEBT-023. No tracking-doc sync gap remains as of this entry.
