...
class TestGoal8ArchitectureClean:
    ...
    def test_no_duplicate_retrieval_or_storage_logic_added(self):
        ...
        decision): core/memory/knowledge_entry.py was added to the allowed
        set. LAYERS["l3"]'s description was corrected from "Graph Memory"
        to "Procedural Memory" to match OCBRAIN_FUTURE_ARCHITECTURE.md's
        own definition of the existing 5-layer system -- a factual
        correction to a dict value, not new logic."""
        import subprocess
        result = subprocess.run(
        ...
        assert core_changes <= {"core/orchestrator.py", "core/memory/unified_memory.py",
                                 ...
                                 "core/memory/knowledge_entry.py", "main.py"}
